  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-2">
          {[{l:"7 Days",v:"7d"},{l:"30 Days",v:"30d"},{l:"90 Days",v:"90d"}].map(p=>(
            <button key={p.v} onClick={()=>setPeriod(p.v)} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${period===p.v?"bg-[var(--indigo)]/15 text-[var(--primary)] border border-[var(--indigo)]/20":"text-[var(--on-surface-variant)] hover:bg-white/5 border border-transparent"}`}>{p.l}</button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {[
          {label:"Total Tickets",value:d.total_tickets,icon:Ticket,trend:"+12%",trendUp:true,color:"var(--indigo)"},
          {label:"Open Tickets",value:d.open_tickets,icon:BarChart3,trend:null,trendUp:false,color:"var(--amber)"},
          {label:"Avg Resolution",value:`${d.avg_resolution_hours}h`,icon:Clock,trend:"-18%",trendUp:true,color:"var(--emerald)"},
          {label:"AI Satisfaction",value:`${d.ai_satisfaction_percent}%`,icon:ThumbsUp,trend:null,trendUp:true,color:"var(--violet)"},
        ].map((card,i)=>(
          <div key={i} className="glass-card p-5 animate-fade-in" style={{animationDelay:`${i*80}ms`}}>
            <div className="flex items-center justify-between mb-3">
              ...
              {card.trend&&(
                <span className={`flex items-center gap-1 text-xs font-semibold ${card.trendUp?"text-[var(--emerald)]":"text-[var(--rose)]"}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 glass-card p-6 animate-fade-in">
          ...
          <div className="space-y-3">
            {Object.entries(d.tickets_by_status).length>0?Object.entries(d.tickets_by_status).map(([s,count])=>{
              ...
              const colors:Record<string,string> = {open:"var(--indigo)",in_progress:"var(--amber)",waiting:"var(--orange,#f97316)",resolved:"var(--emerald)",closed:"var(--outline)"};
              return (
                <div key={s}>
                  ...
                  <div className="h-2 rounded-full bg-white/5 overflow-hidden"><div className="h-full rounded-full transition-all duration-500" style={{width:`${pct}%`,background:colors[s]||"var(--indigo)"}}/></div>
                </div>
              );
        ...
        <div className="glass-card p-6 animate-fade-in">
          ...
          <div className="flex items-center justify-center mb-6">
            <div className="relative w-32 h-32">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                ...
                <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="var(--emerald)" strokeWidth="3" strokeDasharray={`${d.ai_satisfaction_percent}, 100`} strokeLinecap="round"/>
              </svg>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm"><span className="text-[var(--emerald)]">👍 Helpful</span><span className="font-semibold">{d.ai_feedback_summary.helpful}</span></div>
            <div className="flex justify-between text-sm"><span className="text-[var(--rose)]">👎 Unhelpful</span><span className="font-semibold">{d.ai_feedback_summary.unhelpful}</span></div>
